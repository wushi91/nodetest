...
module.exports = {
    ...
    async toGetBook(ctx) {
        ...
        if(type==="renter_phone"){
            ...
            books = await Book.find({renter_phone: renter_phone,is_delete_status:false},{is_delete_status:0,__v: 0})
        }else{
        ...
        for(let i =0;i<books.length;i++){
            ...
            let house = await House.findOne({_id: item.house_id, is_delete_status: false},{openid:0,is_delete_status:0,_id:0,__v: 0})
            item.house = house
            books[i] = item
        }
